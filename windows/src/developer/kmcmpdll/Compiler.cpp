...
KMX_BOOL KMX_CheckContextStatementPositions(PWSTR context) ;
...
WCHAR const * KMX_StoreTokens[TSS__MAX + 2] = {
  L"",
  SSN__PREFIX L"BITMAP",
  SSN__PREFIX L"COPYRIGHT",
  SSN__PREFIX L"HOTKEY",
...
KMX_DWORD KMX_ValidateMatchNomatchOutput(PWSTR p) {
  while (p && *p) {
    if (*p == UC_SENTINEL) {
      switch (*(p + 1)) {
      case CODE_CONTEXT:
...
int KMX_cmpkeys(const void *key, const void *elem)
{
  PFILE_KEY akey, aelem;
  int l1, l2;
  WCHAR char_key, char_elem;
 ...
 * Checks if a use() statement is followed by other content in the output of a rule
 */
KMX_DWORD KMX_CheckUseStatementsInOutput(PWSTR output) {   // I4867
  KMX_BOOL HasUse = FALSE;
  PWSTR p;
  for (p = output; *p; p = incxstr(p)) {
    if (*p == UC_SENTINEL && *(p + 1) == CODE_USE) {
