...
//#include "../../../../developer/kmcompx/include/X/kmx_processevent.h"      // added S
//#include <option.hpp>
//#include <state.hpp>
...
KMX_DWORD KMX_ExpandCapsRule(PFILE_GROUP gp, PFILE_KEY kpp, PFILE_STORE sp);

KMX_DWORD KMX_VerifyCasedKeys(PFILE_STORE sp) {
  assert(sp != NULL);
...
KMX_DWORD KMX_ExpandCapsRule(PKMX_FILE_GROUP gp, PKMX_FILE_KEY kpp, PKMX_FILE_STORE sp) {
  ...
  k = &k[gp->cxKeyArray - 1];
  int a =  std::u16string(kpp->dpContext).length();
  ...
  wcscpy_s(k->dpOutput, std::u16string(kpp->dpOutput).length() + 1, kpp->dpOutput);		// copy the output.
  //k->dpContext = new KMX_WCHAR[wcslen(kpp->dpContext) + 1];     //S
  //k->dpOutput = new KMX_WCHAR[wcslen(kpp->dpOutput) + 1];       //S
  //wcscpy_s(k->dpContext, wcslen(kpp->dpContext) + 1, kpp->dpContext);	// copy the context. //S
  //wcscpy_s(k->dpOutput, wcslen(kpp->dpOutput) + 1, kpp->dpOutput);		// copy the output.  //S
  k->Key = key;
  k->Line = kpp->Line;
  // Add the CAPITAL FLAG, invert shift flag for the rule
  k->ShiftFlags = shift ^ K_SHIFTFLAG | CAPITALFLAG;
